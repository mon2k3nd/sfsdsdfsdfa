...
function WeddingInvitation() {
  ...
  function enterInvitation() {
    if (opening) return;
    setOpening(true);
    window.setTimeout(() => setIntroLeaving(true), 15300);
    window.setTimeout(() => setOpened(true), 17100);
  ...
  return <main className={`paper-texture min-h-screen text-foreground ${opening ? "story-opening" : ""}`}>
        ...
        <div className="intro-story absolute inset-0 z-20 flex items-center justify-center px-7">
          <div className="opening-verse w-full max-w-3xl text-center font-display italic" aria-label={introStory.join(" ")}>
            {introStory.map((line, lineIndex) => (
              <p key={line} aria-hidden className={`opening-verse-line ${lineIndex === introStory.length - 1 ? "opening-verse-finale" : ""}`}>
                {line}
              </p>
            ))}
    ...
    <section id="story" className="mx-auto max-w-4xl px-6 py-12 md:py-18"><div className="reveal"><p className="text-[10px] uppercase tracking-[.3em] text-primary">Chương II · Chuyện chúng mình</p><h2 className="mt-3 text-[2.6rem] leading-[1.1] md:text-5xl">Từ hai câu chuyện,<br/>thành một hành trình</h2></div>
